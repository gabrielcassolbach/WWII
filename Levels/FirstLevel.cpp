...
namespace Levels
{
        ...
        while (quantity > 0)
        {
            ...
            if (nPlayers == 2)
            {
                if (px < 640)
                    ...
                    MovingEntityList.includeEntity(static_cast<Entities::Entity *>(new Entities::Characters::Samurai(1, px, py, 35.0, 60.0, 0.0, 0.0, 6, pPlayersList[1], 1)));
            }
            else
                MovingEntityList.includeEntity(static_cast<Entities::Entity *>(new Entities::Characters::Samurai(1, px, py, 35.0, 60.0, 0.0, 0.0, 6, pPlayersList[0], 1)));
            quantity--;
    ...
    void FirstLevel::createPlayers()
    {
        if (nPlayers == 1)
        {
            if (difficulty == 1)
            {
                Entities::Characters::Player *pPlayer1 = new Entities::Characters::Player(0, 15.0, 500.0, 35.00, 60.0, 0.0, 0.0, 10);
            ...
            {
                Entities::Characters::Player *pPlayer1 = new Entities::Characters::Player(0, 15.0, 500.0, 35.00, 60.0, 0.0, 0.0, 5);
                MovingEntityList.includeEntity(static_cast<Entities::Entity *>(pPlayer1));
                MovingEntityList.includeEntity(static_cast<Entities::Entity *>(pPlayer1->getBullet()));
                pPlayersList.push_back(pPlayer1);
            }
        ...
        else
        {
            if (difficulty == 1)
            {
                ...
                Entities::Characters::Player *pPlayer2 = new Entities::Characters::Player(0, 1220.0, 500.0, 35.00, 60.0, 0.0, 0.0, 10);
    ...
    void FirstLevel::recoverPlayer(ifstream *recover, int color)
    {
        double px, py, vx, vy;
        int id, hp;
        (*recover) >> px >> py >> vx >> vy >> hp;
